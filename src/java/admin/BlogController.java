...
public class BlogController {
    ...
    public static class AdminCreateBlog extends HttpServlet{
    ...
    public static class AdminUpdateBlog extends HttpServlet{
        ...
        protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ...
            if (filePart.getSize() == 0){ //không update ảnh
                ...
                try {
                    String fileName = AdminCreateBlog.getFileName(filePart);
                    assert fileName != null;
                    String newFileName = AdminCreateBlog.generateUniqueFileName(fileName);
                    String uploadDir = req.getServletContext().getRealPath("/") + "uploads";
                    Path filePath = Paths.get(uploadDir, newFileName);
                    try (InputStream fileContent = filePart.getInputStream()) {
                        Files.copy(fileContent, filePath, StandardCopyOption.REPLACE_EXISTING);
    ...
    public static class ViewAllBlogs extends HttpServlet{
        @Override
        protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
            ArrayList<Blog> blogs = new BLogDao().userGetAllBlogs();
            req.setAttribute("blogs", blogs);
